...
from personality_modes import get_mode_style_instructions
from emotion import (
...
def build_character_instructions(
    ...
    mode_style_lines = get_mode_style_instructions(u.get("personality_mode_config"))
    personality_mode_block = (
        "\nPersonality mode style (apply when relevant):\n" + "\n".join(f"- {x}" for x in mode_style_lines)
        if mode_style_lines else ""
    )
...
def generate_llm_voice_reply(
    ...
    if extra_system_hint and extra_system_hint.strip():
