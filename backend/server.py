...
from config import (
    STATE_FILE, MEDIA_ROOT, USER_AUDIO_DIR, ASSISTANT_AUDIO_DIR, VOICE_PROFILES_DIR,
...
def migrate_user_state(u: Dict[str, Any], user_id: str) -> Dict[str, Any]:
    ...
    vp = u.setdefault("voice_profiles", {})
    vp.setdefault("owner", {"display_name": "", "type": "verbal", "enrolled": False, "audio_path": None, "embedding": None})
    vp.setdefault("known", [])
    u["voice_profiles"] = vp

    u.setdefault("personality_mode_config", dict(DEFAULT_MODE_CONFIG))
...
def generate_reply_sequence(
    ...
    if modality == "voice":
        nonverbal_hint = reply_plan.get("nonverbal_prompt_hint")
        reply_text = generate_llm_voice_reply(
            ...
            openai_client,
            OPENAI_MODEL,
            extra_system_hint=nonverbal_hint,
        )
        return [{
            "text": reply_text,
            "modality": "voice",
        ...
        if user_id not in STATE:
            STATE[user_id] = {
                ...
                "delivery_preferences": {
                    ...
                    "respect_user_routine": True,
                    "pseudo_sync_enabled": False,
                    "respond_to_voices": "only_me",
                    "known_speakers": [],
                },
                "voice_profiles": {
                    "owner": {
                ...
                "personality_mode_config": dict(DEFAULT_MODE_CONFIG),
...
def _mime_for_audio_path(file_path: Path) -> str:
    suf = (file_path.suffix or "").lower()
    if suf in (".wav",):
        return "audio/wav"
    if suf in (".mp3", ".mp4", ".m4a"):
        return "audio/mp4" if suf in (".mp4", ".m4a") else "audio/mpeg"
    return "audio/mp4"
...
def transcribe_audio_file(file_path: Path) -> str:
    ...
    mime = _mime_for_audio_path(file_path)
...
def _get_speaker_voice_type(u: Dict[str, Any], speaker_id: Optional[str]) -> Optional[str]:
    """Return voice type ('verbal' or 'nonverbal') for speaker_id, or None."""
    if not speaker_id or speaker_id.strip() in ("", "owner"):
        vp = u.get("voice_profiles", {})
        owner = vp.get("owner", {})
        return owner.get("type") if owner.get("enrolled") else None
    vp = u.get("voice_profiles", {})
    for k in vp.get("known", []):
        if k.get("id") == speaker_id:
            return k.get("type")
    return None
...
def process_user_text_message(
    ...
    if source == "audio" and speaker_id:
        voice_type = _get_speaker_voice_type(u, speaker_id.strip() or None)
        if voice_type == "nonverbal":
            vp = u.get("voice_profiles", {})
            speaker_name = None
            if (speaker_id or "").strip() in ("", "owner"):
                speaker_name = (vp.get("owner") or {}).get("display_name") or u.get("user_name")
            else:
                for k in vp.get("known", []):
                    if k.get("id") == speaker_id:
                        speaker_name = k.get("display_name")
                        break
            event = NonverbalEvent(
                ...
                intensity_score=0.5,
                duration_score=0.5,
                speechlikeness_score=0.6 if user_text.strip() else 0.2,
                distress_score=0.0,
                detected_word=user_text.strip() or None,
                word_confidence=0.7 if user_text.strip() else 0.0,
            )
            result = choose_nonverbal_response(event)
            if result.should_respond:
                reply_plan["nonverbal_prompt_hint"] = result.llm_prompt_hint
...
def _merge_delivery_preferences(u: Dict[str, Any], payload: Dict[str, Any]):
    ...
    if "pseudo_sync_enabled" in payload:
    ...
    if "known_speakers" in payload and isinstance(payload["known_speakers"], list):
        prefs["known_speakers"] = [
            ...
            for s in payload["known_speakers"]
...
# ----- Voice profiles (enrollment for speaker identification) -----

def _voice_embedding_from_audio(audio_path: Path) -> Optional[List[float]]:
    """Extract speaker embedding from audio file. Returns None if extraction not available.
    Best with 16kHz mono WAV; other formats may fail unless resemblyzer/librosa support them."""
    try:
        from resemblyzer import preprocess_wav, VoiceEncoder
        wav = preprocess_wav(str(audio_path))
        if len(wav) < 16000:  # < 1s at 16kHz
            return None
        encoder = VoiceEncoder()
        emb = encoder.embed_utterance(wav)
        return emb.tolist()
    except Exception as e:
        logger.debug("Voice embedding extraction skipped: %s", e)
        return None


@router.get("/voice_profiles/{user_id}")
def get_voice_profiles(user_id: str = Depends(validate_path_user_id)):
    u = get_user(user_id)
    vp = u.get("voice_profiles", {})
    owner = vp.get("owner", {})
    known = vp.get("known", [])
    return {
        ...
        "owner": {
            ...
            "type": owner.get("type", "verbal"),
            "enrolled": bool(owner.get("enrolled", False)),
        },
        "known": [
            {
                "id": k.get("id", ""),
                "title": k.get("title", "") or f"Conhecido {i + 1}",
                "display_name": k.get("display_name", "") or "",
                "type": k.get("type", "verbal"),
                "enrolled": bool(k.get("enrolled", False)),
            }
...
@router.post("/voice_profiles/{user_id}/enroll")
async def enroll_voice_profile(
    user_id: str = Depends(validate_path_user_id),
    profile_id: str = Form(..., description="owner | known_1 | known_2 | ..."),
    display_name: str = Form(""),
    voice_type: str = Form("verbal", description="verbal | nonverbal"),
    ...
    if voice_type not in ("verbal", "nonverbal"):
        voice_type = "verbal"
    ...
    user_dir = VOICE_PROFILES_DIR / user_id
    user_dir.mkdir(parents=True, exist_ok=True)
    ext = Path(getattr(file, "filename", "") or "").suffix or ".wav"
    if ext not in (".wav", ".mp4", ".m4a", ".mp3"):
        ext = ".wav"
    filename = f"{profile_id}_{now_ms()}{ext}"
    ...
    if profile_id == "owner":
        ...
        owner["type"] = voice_type
        owner["enrolled"] = True
        owner["audio_path"] = str(audio_path)
        emb = _voice_embedding_from_audio(audio_path)
        if emb is not None:
            owner["embedding"] = emb
        else:
            owner["embedding"] = None
        vp["owner"] = owner
    else:
        known = vp.get("known", [])
        known_ids = {k.get("id", "") for k in known}
        if profile_id not in known_ids:
            title = f"Conhecido {len(known) + 1}"
            emb = _voice_embedding_from_audio(audio_path)
            known.append({
                "id": profile_id,
                "title": title,
            ...
            for k in known:
                if k.get("id") == profile_id:
                    k["display_name"] = display_name
                    k["type"] = voice_type
                    k["enrolled"] = True
                    k["audio_path"] = str(audio_path)
                    emb = _voice_embedding_from_audio(audio_path)
                    k["embedding"] = emb
                    break
        vp["known"] = known
        prefs = u.setdefault("delivery_preferences", {})
        known_speakers = prefs.get("known_speakers", [])
        ks_ids = {s.get("id", "") for s in known_speakers}
        for k in vp.get("known", []):
            kid = k.get("id", "")
            if kid and kid not in ks_ids:
                known_speakers.append({"id": kid, "name": k.get("display_name", "") or kid})
        prefs["known_speakers"] = known_speakers
        u["delivery_preferences"] = prefs
...
@router.get("/personality_modes/{user_id}")
def get_personality_modes(user_id: str = Depends(validate_path_user_id)):
    u = get_user(user_id)
    config = merge_mode_config(u.get("personality_mode_config"))
    labels = {k: PERSONALITY_MODES.get(k, {}).get("label", k) for k in config}
    return {
        "ok": True,
        "modes": config,
        "labels": labels,
...
@router.post("/personality_modes/{user_id}")
def set_personality_modes(
    user_id: str = Depends(validate_path_user_id),
    payload: Dict[str, Any] = Body(...),
):
    u = get_user(user_id)
    current = dict(u.get("personality_mode_config") or DEFAULT_MODE_CONFIG)
    for key, value in payload.items():
        ...
        if value is None:
            ...
            try:
                current[key] = int(max(0, min(100, float(value))))
            except (TypeError, ValueError):
                pass
    u["personality_mode_config"] = current
    save_state()
    return {
        "ok": True,
        "modes": current,
...
async def send_audio(
    ...
    u = get_user(user_id)
    prefs = u.get("delivery_preferences", {})
    respond_to = prefs.get("respond_to_voices", "only_me")
    speaker = (speaker_id or "").strip() or None
    if speaker and speaker not in ("owner", ""):
        known_ids = {s.get("id", "") for s in prefs.get("known_speakers", []) if s.get("id")}
        if respond_to == "only_me":
            raise HTTPException(status_code=403, detail="Only owner voice is accepted")
        if respond_to == "known_too" and speaker not in known_ids:
            raise HTTPException(status_code=403, detail="Unknown speaker not accepted")
    decay_emotions(u)
    update_drives_passive(u)
    apply_time_update_v2(u)
